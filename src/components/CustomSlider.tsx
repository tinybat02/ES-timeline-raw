...
export class CustomSlider extends Component<IProps, IState> {
  ...
  render() {
    const { domain, mode, step, label, trackLeft, trackRight, defaultTicks, format } = this.props;
    const {
      state: { values, update },
    } = this;
    ...
    return (
      <div style={{ display: 'flex', marginBottom: 10 }}>
        <span style={{ fontWeight: 'bold', width: 100, paddingTop: 7 }}>{label}</span>
        {update.length == 1 ? (
          <span
            style={{ padding: '7px 0px 10px 0px', marginRight: 20, fontWeight: 'bold', width: 40, textAlign: 'right' }}
          >
            {format(update[0])}
          </span>
        ) : (
          ...
          >
            <div style={{ fontWeight: 'bold', textAlign: 'right' }}>{format(update[0])}</div>
            <div style={{ fontWeight: 'bold', textAlign: 'right' }}>{format(update[1])}</div>
          </div>
        )}
